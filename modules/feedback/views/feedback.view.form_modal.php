...
echo '
<div class="feedback_form formmodal">
...
if(! empty($result["rows"]))
{
	foreach ($result["rows"] as $row) //вывод полей из конструктора форм
	{
		...
		switch ($row["type"])
		{
			...
			case "images":
				echo '<div class="infofield">'.$row["name"].($row["required"] ? '' : '').'</div><div class="images"></div>';
...
//Кнопка Отправить
echo '<div class="formmodal__submit"><input type="submit" value="'.$this->diafan->_('Заказать звонок', false).'" class="button solid">';

echo '<div class="privacy_field">'.$this->diafan->_('Нажимая на кнопку отправить, я даю свое согласие на обработку <a href="%s">персональных данных</a>.', true, BASE_PATH_HREF.'privacy'.ROUTE_END).'</div></div>';
