...
if ($result["row"]["price_arr"])
{
	foreach ($result["row"]["price_arr"] as $price)
	{
		...
		echo '<div class="js_shop_param_price shop_param_price shop-item-price"'.$param_code.'>';
			...
			{
				echo '<span class="shop_old_price price-old"><span class="shop_price_value strike"><span class="product__price" style="    color: rgb(151, 152, 160);    text-decoration: line-through;">'.$price["old_price"].' '.$result["result"]["currency"].'</span></span></span>';
			}
			
			echo '<span class="price"><span class="product__price"><span class="js_shop_price" summ="'.$price["price_no_format"].'" format_price_1="'.$this->diafan->configmodules("format_price_1", "shop").'" format_price_2="'.$this->diafan->configmodules("format_price_2", "shop").'" format_price_3="'.$this->diafan->configmodules("format_price_3", "shop").'">'.$price["price"].'</span> '.$result["result"]["currency"].'</span></span>';
...
echo '<div class="js_shop_buy shop_buy to-cart">';
	...
	{
		echo '<input type="text" value="1" name="count" class="number"  pattern="[0-9]+([\.|,][0-9]+)?" step="any" style="display: none;">';
	...
	{
		echo '<div class="product__buy"><input type="button" class="button solid addcart" value="'.$this->diafan->_('Купить', false).'" action="buy"></div>';
	}
echo '</div>';
